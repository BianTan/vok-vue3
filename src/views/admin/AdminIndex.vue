<template>
  <div class="flex mx-auto" :class="switchSmall">
    <sidebar
      class="transform lg:translate-x-0 top-0 left-0 fixed lg:sticky"
      :class="[{ '-translate-x-full ': !isMenuShow }]"
    >
      <sidebar-item
  ...
  setup() {
    ...
    watchEffect(() => {
      if (index.value) {
        let id = index.value
        if (
          (route.query.post_type && route.query.post_type === 'page') ||
          (route.query.page && route.name === 'adminPost')
        ) {
          id = 3
        }
    ...
    const handleItemClick = () =>
      setTimeout(() => (isMenuShow.value = false), 300) // 点击侧边栏菜单延迟 300 毫秒关闭
    return {
      menuList,
      switchSmall,
      userData,
