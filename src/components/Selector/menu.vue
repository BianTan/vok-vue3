<template>
  <div class="absolute top-10 left-0 w-full px-3 z-20">
    <ul class="bg-white shadow rounded-md overflow-hidden text-center">
      <li class="text-sm h-10 leading-10 cursor-pointer hover:bg-opacity-80 hover:bg-gray-100 transition ease-in-out duration-300"
        v-for="item in searchData"
        :key="item.id"
        @click="itemClick([item.value, item.text])">{{item.text}}</li>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    onMounted(() => {
      searchData.value = props.data // 初始化搜索数据为粑粑传下来的 data
    })
    const dataFilter = (value: string) => { // 过滤方法
      if(props.data) {  // 如果存在 data
        return props.data.filter((f: DataProps) => {
    ...
    watch(() => props.searchValue, () => {  // 监听 input 框的 value 是否有变化
      searchData.value = dataFilter(props.searchValue)  // 调用过滤方法
    })
