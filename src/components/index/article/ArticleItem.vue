<template>
  <article
    v-if="postData.type === 0"
    class="bg-white rounded overflow-hidden shadow pb-6"
    :class="[{ 'pt-6': postData.post_url.length === 0 }]"
  >
    <router-link
      :to="`/post/${postData.id}.html`"
      class="aspect-w-12 aspect-h-6 sm:aspect-h-5 lg:aspect-h-4 md:aspect-h-3 block mb-3"
      v-if="postData.post_url.length !== 0"
    >
      <img
    ...
    <router-link
      class="px-8 mt-1 mb-3 text-xl sm:text-2xl inline-block"
      :to="`/post/${postData.id}.html`"
    ...
    <info-list class="mx-8 mt-2 pb-2">
      ...
      <info-item iconName="category"
        ><router-link :to="`/category/${postData.category.id}`">{{
  ...
  <article v-else-if="postData.type === 1" class="sm:flex">
    ...
    <card class="flex-1 relative sm:ml-10 px-7 py-5">
      <i
        class="iconfont icon-quote text-white bg-blue-800 w-8 h-8 rounded-full flex items-center justify-center absolute -top-4 left-2 sm:-top-3 sm:-left-4"
      ></i>
      <p class="text-gray-700" v-html="postData.description"></p>
    </card>
  </article>
</template>
