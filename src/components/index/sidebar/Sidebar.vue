<template>
  <div class="space-y-4">
    <card class="py-10 px-6">
      ...
      <div
        v-if="isPost"
        class="flex text-sm list-none mt-4 overflow-hidden rounded-t border-b border-gray-100"
      >
        <sidebar-item
          v-for="(item, index) in switchItem"
          :key="index"
          @click="switchClick(index)"
          :isActivate="index === currentIndex"
          >{{ item }}</sidebar-item
        >
      </div>
      <div
        class="m-3"
        :class="[{ 'border-t border-gray-100 mt-6 pt-3': !isPost }]"
      >
